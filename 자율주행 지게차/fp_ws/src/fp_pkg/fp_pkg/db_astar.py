...
class ReturnManager:
    ...
    def _calc_home_path_async(self):
        ...
        if self.transition(RobotState.RUNNING_TO_HOME):
            drive_msg = Bool(); drive_msg.data = True   # 추가
            node.pub_mission_start.publish(drive_msg)    # 추가
    ...
    def on_unload_done(self) -> bool:
        ...
        if pending:
            ...
            self._node.stop_robot()
            # [BUG FIX] 하차(파란불) 상태에서 귀환으로 전환 시 즉시 HOME(초록불) 발행.
            # 기존: _on_home_arrived()에서만 HOME 발행 → 귀환 내내 UNLD(파란불) 유지되던 문제 수정.
            cmd_home = String()
            cmd_home.data = ArduinoCmd.HOME
            self._node.pub_forklift_cmd.publish(cmd_home)
            self._node.get_logger().info("[Serial] Sent HOME command to Arduino → 귀환 LED(초록) 전환")
            if self.transition(RobotState.ABORTING):
                self._retry_count = 0
                self._start_home_path_thread()
            return True
    ...
    def _set_destination_safe(self, goal_pose: list, node_key: str = ''):
        ...
        node.publish_path_viz()
        drive_msg = Bool(); drive_msg.data = True        # 추가
        node.pub_mission_start.publish(drive_msg)
...
class IntegratedNavigation(Node):
    def __init__(self):
        ...
        self._emergency_source = None          # [BUG FIX] 중복 초기화 제거, 1회만 선언

        # ── 하차 작업 플래그 ──
        self.is_unload_task_active = False     # [BUG FIX] 미초기화 변수 추가
        # 자신이 발행한 /unload_task_start 메시지를 자신이 받는 것을 막기 위한 플래그
        self._unload_sig_self_published = False  # [BUG FIX] 순환 pub/sub 방지
        ...
        # [BUG FIX] return_home_publisher, emergency_resolve_web_publisher 중복 퍼블리셔 제거
        ...
        # [BUG FIX] 순환 pub/sub 제거:
        # 기존: pub_unld_sig → '/get_unload' 발행 후 동일 토픽을 sub_unld_sig 로 수신 → 자기 메시지 처리
        # 수정: FSMUnloadNode가 하차 시작을 알릴 별도 토픽('/unload_task_start') 사용
        self.sub_unld_task_start = self.create_subscription(
            String, '/unload_task_start',
            self._unload_task_start_callback, volatile_profile)

        # 공통 노드 초기 로드 (시작 시 1회 — 이후 주행 요청마다 갱신)
        self._reload_common_nodes()
    ...
    # ── [NEW] DB에서 공통 노드 재로드 ──
    def _reload_common_nodes(self):
        """
        맵 교체·노드 변경 후에도 최신 DB 정보를 반영하기 위해
        주행 시작 신호 수신 시마다 호출한다.
        """
        try:
            ...
            self.get_logger().info(
                f"✅ 공통 노드 갱신: {list(common.keys())} | 경유 노드 {len(waypoints)}개")
        except Exception as e:
            self.get_logger().warn(f"⚠️  공통 노드 로드 실패 (기존 노드 유지): {e}")

    # ── [BUG FIX] /unload_task_start 전용 콜백 (순환 pub/sub 해결) ──
    def _unload_task_start_callback(self, msg: String):
        """FSMUnloadNode가 '/unload_task_start' 토픽으로 하차 시작을 알릴 때 호출"""
        ...
        if cur in (RobotState.MISSION, RobotState.IDLE):
            self.return_mgr.transition(RobotState.UNLOADING)
        else:
            self.get_logger().warn(
                f"[Nav] UNLOADING 전환 불가 (현재 상태: {cur})")
    ...
    def goal_pose_callback(self, msg):
        ...
        # [BUG FIX] 주행 신호 수신 시 DB에서 최신 노드 갱신
        self._reload_common_nodes()
        if self.waypoints:
            self.return_mgr.build_return_graph(self.waypoints)
    ...
    def _resolve_emergency(self):
        ...
        if self.return_mgr.get_state() == RobotState.SAFE_EXIT:
            ...
            # [BUG FIX] 중복 publish 제거 — 1회만 발행
            done_msg = Bool(); done_msg.data = True
            self.pub_emergency_resolved.publish(done_msg)
            self.get_logger().info("✅ 비상정지 해제 완료 → /emergency_resolved 발행")
    ...
    def done_callback(self, msg):
        ...
        if self.return_mgr.on_unload_done():
            self.is_waiting = False
            self.is_unload_task_active = False
            return
    ...
    def _advance_mission(self):
        # 경유 주행 모드
        if self.is_waypoint_loop:
            # 경유 노드 없이 직접 최종 목적지로 가는 경우 — 도착은 control_loop에서 처리
            ...
            if next_idx < len(self.waypoints):
                ...
                self.return_mgr.update_node_key('wp_final')
                # [BUG FIX] wp_final_pose None 체크
                if self.wp_final_pose is None:
                    self.get_logger().error("❌ wp_final_pose 없음 — 경유 주행 종료")
                    self.is_waypoint_loop = False
                    self.return_mgr.transition(RobotState.IDLE)
                    return
    ...
    def _full_reset(self):
        ...
        self._emergency_source = None
        self.is_unload_task_active = False
        self._unload_sig_self_published = False
        self.is_initial_turn = False
        self.wp_final_aligning = False
        self._waiting_stop_sent = False
        self._battery_warn_sent = False
    ...
    def trigger_mission(self, unload_zone: int = 1):
        ...
        # [BUG FIX] 미션 시작마다 DB에서 최신 노드 로드 (맵 교체/노드 변경 반영)
        try:
            nav_db = NavigationDBHelper()
    ...
    def set_next_destination(self, goal_pose):
        # [BUG FIX] goal_pose None 체크
        if goal_pose is None:
            self.get_logger().error("❌ set_next_destination: goal_pose가 None입니다!")
            return
    ...
    def control_loop(self):
        ...
        if dist_to_goal < self.stop_tolerance:
            ...
            if self.is_waypoint_loop:
                ...
                # [BUG FIX] 경유 노드 없을 때 len(self.waypoints)-1 = -1 이 되는 문제 수정
                # 최종 목적지 도착 여부를 명확하게 판단
                at_final = False
                if self.wp_final_pose is not None:
                    dist_to_final = sqrt(
                        (self.current_pose[0] - self.wp_final_pose[0])**2 +
                        (self.current_pose[1] - self.wp_final_pose[1])**2)
                    at_final = dist_to_final < self.stop_tolerance

                if at_final or (not self.waypoints):
                    # 경유 노드 없이 바로 최종 목적지 도착한 경우 포함
                    self.stop_robot()
                    self.global_path = []
                    self.wp_final_aligning = True
            ...
            # 하차 도착
                ...
                # [BUG FIX] 자신의 pub을 자신이 수신하지 않도록 플래그 설정 후 발행
                # (FSMUnloadNode는 /get_unload 를 수신해서 작업 시작,
                #  작업 시작 알림은 /unload_task_start 토픽으로 별도 수신)

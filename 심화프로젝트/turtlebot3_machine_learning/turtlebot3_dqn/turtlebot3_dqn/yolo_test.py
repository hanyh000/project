...
class RLEnvironment(Node):
    ...
    def __init__(self):
        ...
        self.target_distance = 0.0  # 추가
        self.center_offset = 0      # 로그에서 center_offset도 쓰고 있으니 같이 추가
        self.target_detected = False # 추가
        ...
        self.rotation_cooldown_steps = 30   # 3~5 권장
        ...
        # [추가] calculate_state에서 사용할 ID들 초기화
        self.cp_idx = 0     # 현재 미션 번호
        self.start_idx = 0  # 시작 지점 번호 (State 전달용)
    ...
    def camera_sub_callback(self, msg):
        ...
        # 4. 화면에 출력 (디버깅용 창 생성)
        # ※ 주의: headless 환경(터미널만 있는 경우)에서는 에러가 날 수 있습니다.
        #cv2.imshow("YOLO Detection Debug", debug_image)
        #cv2.waitKey(1)

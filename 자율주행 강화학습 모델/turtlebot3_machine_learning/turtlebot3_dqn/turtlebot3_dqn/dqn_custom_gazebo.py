...
class GazeboInterface(Node):
    ...
    def __init__(self, stage_num):
        ...
        self.entity_pose_x = 2.0
        self.entity_pose_y = 1.8
    ...
    def reset_simulation(self):
        ...
        rclpy.spin_until_future_complete(self, future)
        self.get_logger().info('Gazebo Simulation Reset 완료')

        self.unpause_physics_client.call_async(Empty.Request())
        self.get_logger().info('Gazebo Physics Re-started')
    ...
    def task_succeed_callback(self, request, response):
        self.get_logger().info('Task Succeed! Resetting for next episode...')
        self.delete_entity()
        time.sleep(0.05)
        if ROS_DISTRO == 'humble':
